...
exports.clean = async function (args) {
    ...
    if (args.action === "slide") {
        ...
        child_process.execSync(`ffmpeg -loop 1 -i "${args.slidename}" -i "${args.mp3name}" -c:a copy -c:v libx264 -shortest -v quiet -y "../video/${args.target}"`);
        ...
        child_process.execSync(`ffmpeg -f concat -safe 0 -i "filelist.txt" -c copy -v quiet -y "../dist/${args.target}"`);
