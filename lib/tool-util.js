...
let conf = {
    ...
    taskImg2Mp4: [], // 图和mp3生成视频片段的任务在此
    taskMp42Mp4: [], // 合并视频片段的过程任务在此
    dict: null,
    voice: {
        cn: { female: "zh-CN-XiaoyanNeural", male: "zh-CN-YunyangNeural" },
        en: { female: "en-US-SaraNeural", male: "en-US-SteffanNeural" }
    }
};
...
let util = {
    ...
    initMaterial: async function () {
        ...
        console.log("读取到内容数据 " + ret.data.length + " 条");
        conf.dict = await util.fetchApi("lib/dict.json");
        console.log("读取到字典数据 " + Object.keys(conf.dict).length + " 条");

        ui.putInputData("theme", localStorage.getItem("theme") || "lc"); // 用localstorage里的值填入
        ui.putRadioValue("lesson", localStorage.getItem("lesson") || "chinese"); // 用localstorage里的值填入
