...
let util = {
    ...
    checkMedias(line, fields, medias) {
        for (let field of fields) {
            for (let media of medias) {
                if (util.checkMediaStarus(line, field, media)) {
    ...
    checkMediaStarus(line, field, media) {
        ...
        if (key === "media_cn2.audio") return (has("cn1.listen") || has("cn1.text")) && line.voice === "";
    ...
    getMaterialForExport(type) {
        if (type === "Template") {
            ...
            let material = Object.values(conf.materials).map((line) => conf.dataFields.map((field) => (line[field] !== 0 ? line[field] : "")));
            let courseware = [];
            for (let line of Object.values(conf.materials)) {
                util.isLessonEnglish()
                    ? courseware.push([line.id.toString()], [line.english + line.phonetic], [line.chinese], [""])
                    : courseware.push([line.id.toString(), line.phonetic], ["", line.chinese], ["", line.english], [""]);
            }
            return [[conf.exportTitles].concat(material), courseware];
        }
    },
    ...
    getXlsxBinary(maerial, courseware) {
        ...
        XLSX.utils.book_append_sheet(workbook, maerial_sheet, `${conf.info.theme} 素材`);

        // 如果有课件，添加课件sheet
        if (courseware) {
            let courseware_sheet = XLSX.utils.aoa_to_sheet(courseware);
            courseware_sheet["!cols"] = [{ wpx: 25 }, { wpx: 490 }];
            ...
            XLSX.utils.book_append_sheet(workbook, courseware_sheet, `${conf.info.theme} 课件`);
        }
