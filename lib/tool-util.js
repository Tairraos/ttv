...
let util = {
    ...
    // 计算出使用的背景图片名字
    /*********************/
    getTheme: (projectid, lesson) => `${conf.lesson[lesson].abbr}-${(+projectid).toString().padStart(3, "0")}`,
    ...
    fmtDuration: (s) => ("0" + Math.floor(s / 60)).slice(-2) + ":" + ("0" + Math.floor(s % 60)).slice(-2) + (s % 1).toFixed(3).slice(1),
        ...
        if (type === "Template") {
            ...
            for (let line of materials) {
                util.isLessonEnglish()
                    ? exportCourseware.push([line.id.toString()], [line.english + line.phonetic], [line.chinese], [""])
                    : exportCourseware.push(["", line.phonetic], [line.id.toString(), line.chinese], ["", line.english], [""]);
            }
