...
let util = {
    ...
    async checkTableData() {
        for (let line of util.getMaterial()) {
            util.updateMedia(line.id, "media_cn1", "audio", util.isLessonChinese() || line.type !== "word");
            util.updateMedia(line.id, "media_cn1", "video", util.isLessonChinese() || line.type !== "word");
            util.updateMedia(line.id, "media_cn2", "audio", (util.isLessonChinese() || line.type !== "word") && line.voice === "");
            util.updateMedia(line.id, "media_cn2", "video", (util.isLessonChinese() || line.type !== "word") && line.voice === "");
            util.updateMedia(line.id, "media_en1", "audio", true);
            util.updateMedia(line.id, "media_en1", "video", true);
            util.updateMedia(line.id, "media_en2", "audio", line.voice === "");
            util.updateMedia(line.id, "media_en2", "video", line.voice === "");
            util.updateMedia(line.id, "slide", "text", true);
            util.updateMedia(line.id, "slide", "ding", true);
            conf.materials[line.id].isready = util.checkIsReady(line.id);
            ui.updateCell(line.id, "check", "isready", conf.materials[line.id].isready ? "ready" : "unready");
        }
    },

    updateMedia(id, field, type, condition) {
        if (!condition) {
            conf.materials[id][`${field}.${type}`] = null;
            return ui.updateCell(id, field, type, "ignored");
        }
        let filename = `${id}.${field}${type}`
            .replace(/audio$/, ".m4a")
            .replace(/video$/, ".mp4")
            .replace(/slidetext$/, "text.png")
            .replace(/slideding$/, "ding.png");
        if (conf.files.includes(filename)) {
            conf.materials[id][`${field}.${type}`] = filename;
            ui.updateCell(id, field, type, "exist");
        } else {
            conf.materials[id][`${field}.${type}`] = "required";
            ui.updateCell(id, field, type, "required");
        }
    },
    ...
    getMediaSetup(id, language) {
        let line = conf.materials[id],
            lang = language === "chinese" ? "cn" : "en";
        if (util.isLessonEnglish() && language === "chinese" && line.type === "word") {
            return []; //英语课，单词的中文，不用生成音频
        } else if (line.voice !== "") {
            return [{ model: util.getModel(language, line.voice), target: `media_${lang}1` }]; // 指定了voice
        }
        return [
            { model: util.getModel(language, "man"), target: `media_${lang}1` }, // 未指定voice, 就男女各生成音频
            { model: util.getModel(language, "woman"), target: `media_${lang}2` }
        ];
    },
