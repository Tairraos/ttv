...
let util = {
    ...
    checkMaterials() {
        ...
        for (let line of util.getMaterial()) {
            util.checkMedia(line.id, "media_cn1", ["audio", "video-text", "video-listen"], true); // 所有条件都有英文第一套素材
            util.checkMedia(line.id, "media_cn2", ["audio", "video-text", "video-listen"], line.voice === ""); // 指定voice，没有第二套音频
    ...
    getXlsxBinary(content) {
        let workbook = XLSX.utils.book_new(),
            ...
            widths = conf.exportList.map((item) => item.width);

        worksheet["!cols"] = widths.map((i) => ({ wpx: i })); // 调整每一列宽度
