...
function getCell(line, field, media) {
    let status = media === "theme" ? true : util.checkMediaStatus(line, `${field}.${media}`),
        filename = media === "theme" ? `${theme}.png` : line.id + "/" + util.getMediaFilename(line.id, field, media),
        is_exist = file_exists(filename),
        mediaString;
    if (media.match(/audio/)) {
        mediaString = `<audio controls src="media/${book_cn}/audio/${filename}"></audio>`;
    } else if (media.match(/video/)) {
        mediaString = `<video controls src="media/${book_cn}/video/${filename}"></video>`;
    } else if (media.match(/slide/)) {
        mediaString = `<img src="media/${book_cn}/slide/${filename}" />`;
    } else {
        mediaString = `<img src="media/${book_cn}/theme/${filename}" />`;
    }
    return [
        `<td class="media">`,
...
for (let line of lines) {
    $media.innerHTML += `<tr><td colspan="4" class="label">id: ${line[`id`]}, 类型: 语音, 中文: [ ${line[`chinese`]} ], 英文: [ ${
        line[`english`]
    } ]</td></tr><tr>`;
    $media.innerHTML += `<tr>${["media_cn1", "media_cn2", "media_en1", "media_en2"].map((field) => getCell(line, field, "audio")).join("")}</tr>`;
    $media.innerHTML += `<tr><td colspan="4" class="label">id: ${line[`id`]}, 类型: 字幕视频, 中文: [ ${line[`chinese`]} ], 英文: [ ${
        line[`english`]
    } ]</td></tr><tr>`;
    $media.innerHTML += `<tr>${["media_cn1", "media_cn2", "media_en1", "media_en2"].map((field) => getCell(line, field, "video-text")).join("")}</tr>`;
    $media.innerHTML += `<tr><td colspan="4" class="label">id: ${line[`id`]}, 类型: 听力视频, 中文: [ ${line[`chinese`]} ], 英文: [ ${
        line[`english`]
    } ]</td></tr><tr>`;
