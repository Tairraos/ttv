...
let action = {
    ...
    doImportMaterials(file) {
        ...
        reader.onload = async function (e) {
            ...
            ui.log(`0.导入完成，共有${range.e.r - 1}条语料`, "pass");
    ...
    async doGenAudioPiece() {
        await action.genSetupAudioPiece(conf.editTool.id, conf.editTool.language, true); //force=true,覆盖生成
    },
    ...
    async doBuild() {
        ...
        let log = ui.log(`生成作品：${conf.info.theme}.mp4`, "highlight");
        let ret = await util.fetchApi("api/ffmpeg", { action: "video", filename: `${conf.info.theme}.mp4`, videolist: conf.tasks.join("|") });
        if (ret.result === "success") {
            ui.done(log);
            ...
            return ui.err(`生成作品 ${conf.info.theme}.mp4 时遇到错误`);
        }
        ui.log(`7.作品已经生成`, "pass");
        util.getProjectid();
        window.open(`media/material/dist/${conf.info.theme}.mp4`, "preview");
    },
    /***********************/
    // 8.存档数据
    /***********************/
    async doArchive() {
        ...
        await util.fetchApi("api-archive.php", { lesson });
        ui.done(log);
