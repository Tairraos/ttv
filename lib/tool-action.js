...
let action = {
    ...
    // 同步project表数据
    /*********************/
    syncProject: async function (projectid, lesson, theme, duration = 0) {
    ...
    doGenAudio: async function () {
        ...
        for (let line of util.getMaterial(() => true)) {
            ...
            if (!line.cn_male && !(util.isLessonEnglish() && line.type === "word")) {
                await action.createAudio(line.id, line.chinese, voice.cn_male, `${line.id}.cn_male`, "cn_male");
            }
            if (!line.cn_female && !(util.isLessonEnglish() && line.type === "word")) {
                await action.createAudio(line.id, line.chinese, voice.cn_female, `${line.id}.cn_female`, "cn_female");
            }
            if (!line.en_male) {
                await action.createAudio(line.id, line.english, voice.en_male, `${line.id}.en_male`, "en_male");
            }
            if (!line.en_female) {
                await action.createAudio(line.id, line.english, voice.en_female, `${line.id}.en_female`, "en_female");
            }
        }
    },

    createAudio: async function (id, content, voice, filename, field) {
        let log = ui.log(`使用${voice}生成MP3：${content} -> ${filename}.m4a`),
            ret = await util.fetchApi("api/tts", { filename, text: content, voice });
        if (ret.result === "success") {
            await util.updateMaterial(id, `${filename}.m4a`, field);
            ui.done(log);
        } else {
    ...
    doCaptureSlide: async function () {
        for (let line of util.getMaterial((line) => !line.slide)) {
            let log,
                filename,
                watermark = ((line.id / 10) | 0) % 4,
                theme = conf.info.theme,
                lesson = util.getLesson();
            filename = `${line.id}.text.png`;
            log = ui.log(`生成slide：${filename}`);
            await util.fetchApi("api/slide", { filename, theme, lesson, type: "text", watermark, id: line.id });
            await util.updateMaterial(line.id, filename, "slide");
            ui.done(log);
            filename = `${line.id}.ding.png`;
            log = ui.log(`生成slide：${filename}`);
            await util.fetchApi("api/slide", { filename, theme, lesson, type: "slide", watermark, svg: +new Date() % 4 });
            ui.done(log);
        }
    ...
    genNoimgSlide: async function () {
        ...
        for (let id of [1, 2, 3, 4]) {
            ...
            await util.fetchApi("api/slide", { id, filename, theme: conf.info.theme, lesson: util.getLesson(), type: "slide" });
