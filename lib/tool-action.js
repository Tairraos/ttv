...
let action = {
    ...
    doGenPhonetic: async function () {
        ...
        for (let line of util.getMaterial((line) => line.english && conf.info.lesson === "english" && line.type === "word" && !line.phonetic)) {
    ...
    doGenAudio: async function () {
        for (let line of util.getMaterial(() => true)) {
            if (line.group !== conf.voice.groupid || line.group === 0) {
                // Group变化，或没有Group, 则随机选一组男女语音，如果是同一个group的成员，语音模型不变
                conf.voice.groupid = line.group;
                conf.cn_female = conf.cnlib.female[Math.floor(Math.random() * conf.cnlib.female.length)];
                conf.cn_male = conf.cnlib.male[Math.floor(Math.random() * conf.cnlib.male.length)];
                conf.en_female = conf.enlib.female[Math.floor(Math.random() * conf.enlib.female.length)];
                conf.en_male = conf.enlib.male[Math.floor(Math.random() * conf.enlib.male.length)];
            }

            if (!line.cn_male && !(conf.info.lesson === "english" && line.type === "word")) {
                await action.createMp3(line.id, line.chinese, conf.voice.cn_male, `${line.id}.cn_male.mp3`, "cn_male");
            }
            if (!line.cn_female && !(conf.info.lesson === "english" && line.type === "word")) {
                await action.createMp3(line.id, line.chinese, conf.voice.cn_female, `${line.id}.cn_female.mp3`, "cn_female");
            }
            if (!line.en_male) {
                await action.createMp3(line.id, line.english, conf.voice.en_male, `${line.id}.en_male.mp3`, "en_male");
            }
            if (!line.en_female) {
                await action.createMp3(line.id, line.english, conf.voice.en_female, `${line.id}.en_female.mp3`, "en_female");
            }
        }
    },
    ...
    doGenVideo: async function () {
        ...
        if (ret.result === "success") {
            ...
            await util.fetchApi("api-project.php", { action: "update", projectid: conf.info.theme, duration: ret.duration });
        } else {
    ...
    doArchive: async function () {
        let log = ui.log(`拼接完整视频：${conf.info.theme}.mp4`, "highlight");
        await util.fetchApi("api-archive.php", { projectid: conf.info.theme });
        ui.done(log);
    }
};
