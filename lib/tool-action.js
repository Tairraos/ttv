...
let action = {
        ...
        reader.onload = async function (e) {
            ...
            for (let line of lines) {
                if (line.match(/^~word/)) {
                    type = "word";
                    ui.log(`词汇模式`);
                } else if (line.match(/^~sentense/)) {
                    type = "sentense";
                    ui.log(`句子模式`);
                } else if (line.match(/^~practice/)) {
                    type = "practice";
                    ui.log(`练习模式`);
                } else if (line.match(/^\{/)) {
                    group = conf.maxid + 1;
    ...
    doPrepare: async function () {
        ...
        await util.fetchApi("api/slide", { filename: `${conf.info.theme}.notext.png`, theme: conf.info.theme, lesson: conf.info.lesson });
        conf.info.notextImg = `${conf.info.theme}.notext.png`;
        ui.done(log);
