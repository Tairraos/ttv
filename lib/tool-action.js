...
let action = {
    ...
    async genVideoPiece(id, field, force = false) {
        if (force || conf.materials[id][`${field}.video`] === `required`) {
            // let filename = `${id}.${field}.mp4`,
            // audioname, slidename=conf.materials[id][`${field}.video`];
            // let log = ui.log(`生成视频片段：${filename}`);
            // await util.fetchApi("api/ffmpeg", { id, filename, theme, language, type, watermark });
            // await util.updateMaterial(id, filename, "mediafile");
            // ui.done(log);
        }
    },
