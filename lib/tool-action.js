...
let action = {
        ...
        reader.onload = async function (e) {
            ...
            for (let row = 2; row <= range.e.r + 1; row++) {
                if (g(`A${row}`) !== "") {
                    ...
                    let ret = await util.fetchApi("api-material.php", { id, lesson, type, voice, group, chinese, english, phonetic, comment, theme });
    ...
    async doBuild() {
        ...
        let ret = await util.fetchApi("api/ffmpeg", { action: "video", filename: `${conf.info.dist}.mp4`, videolist: conf.tasks.join("|") });
        if (ret.result === "success") {
            ui.done(log);
