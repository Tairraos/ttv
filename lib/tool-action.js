...
let action = {
    ...
    async genAudioPiece(id, language, force = false) {
        let text = ui.getCell(id, language).innerText,
            setup = util.getMediaSetup(id, language);
        for (let item of setup) {
            if (force || conf.materials[id][`${item.target}.audio`] === `required`) {
    ...
    async doCaptureSlide() {
        for (let line of util.getMaterial()) {
            action.genSlidePiece(line.id);
        }
    },

    async genSlidePiece(id, force = false) {
        (force || conf.materials[id][`slide.text`] === `required`) && (await action.createSlide(id, "text"));
        (force || conf.materials[id][`slide.ding`] === `required`) && (await action.createSlide(id, "ding"));
    },

    async createSlide(id, type) {
        let log,
            watermark = ((id / 16) | 0) % 4, // 每16行语料，水印换一个位置
            theme = conf.info.theme,
            language = util.getLanguage(),
            filename = `${id}.${type}.png`;
        log = ui.log(`生成slide：${filename}`);
        await util.fetchApi("api/slide", { filename, theme, language, type, watermark, id });
    ...
    async checkTableData() {
        for (let line of util.getMaterial()) {
            ...
            if (fp.match(/^.....11.(1111|1010|0101)1.$/)) {
                !line.check && (await util.updateMaterial(line.id, "yes", "check"));
                row.classList.add("ready");
                ui.done(log);
            } else {
                line.check && (await util.updateMaterial(line.id, "", "check"));
