...
let action = {
    ...
    // 同步project表数据
    /*********************/
    syncProject: async (projectid, lesson, theme, duration = 0) => {
    ...
    fetchTranslation: async (from, to) => {
    ...
    createAudio: async (id, content, voice, filename, field) => {
    ...
    doCaptureSlide: async () => {
        for (let line of util.getMaterial((line) => !line.slide)) {
            ...
            log = ui.log(`生成slide：${filename}`);
            await util.fetchApi("api/slide", { filename, theme, lesson, type: "slide", watermark, svg: +new Date() % 4 }); // 随机 svg
            ui.done(log);
        }
    },
