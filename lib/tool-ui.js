...
let ui = {
    ...
    doPreviewMaterials(event) {
        ...
        if (dom.className.match(/^media/)) {
            let mediaType = dom.textContent.match(/m4a$/) ? "audio" : "video";
            window.open(`media/material/${mediaType}/${dom.textContent}`, "preview");
        } else if (dom.className.match(/^id$/)) {
            window.open(`api-page.php?id=${row.dataset.id}&theme=${conf.info.theme}&language=${util.getLanguage()}`, "preview");
    ...
    switchEditTool() {
        ...
        document.getElementById("doEditDone").style.display = !conf.edit.locker ? "none" : "block";
        document.getElementById("doEditRestore").style.display = !conf.edit.locker ? "none" : "block";
    },
...
$domMaterials.addEventListener("mouseover", ui.doSubMethod, false);
