/* global conf, util, fieldsList, action*/
...
let ui = {
    ...
    syncProject: async function () {
        let ret = await action.syncProject(ui.getInputData("projectid") || "0001", ui.getSelectData("lesson"));
        conf.maxid = +ret.maxid;
        ui.putInputData("projectid", ret.data.projectid.toString().padStart(4, "0")); // 用请求返回里的值填入
    ...
    // 生成导出模板按钮
    /*********************/
    updateDownloadLink: function (content, id,filename) {
        let link = document.getElementById(id || "downloadTemplate"),
            binary = util.getXlsxBinary(content);
...
/*********************/
document.getElementById("projectid").addEventListener("input", ui.onProjectChange, false);
document.getElementById("projectid").addEventListener("paste", ui.onProjectChange, false);
document.getElementById("lesson").addEventListener("change", ui.onProjectChange, false);
...
document.getElementById("downloadTemplate").addEventListener("click", action.downloadTemplate, false);
